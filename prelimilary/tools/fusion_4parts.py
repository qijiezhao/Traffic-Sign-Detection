...
file_source='test_crop4_dot01_nms45.txt'
lines=open(file_source,'r').readlines()
file_target='fusion4parts_test_crop4_dot01_nms45_ov45.txt'
fw=open(file_target,'w')

def get_side(subimage_name):
    if subimage_name[6]=='a'and subimage_name[7]=='x':
        tmp_side=0
    elif subimage_name[6]=='a'and subimage_name[7]=='y':
        tmp_side=1
    elif subimage_name[6]=='b'and subimage_name[7]=='x':
        tmp_side=2
    else:
        tmp_side=3
    image_name=subimage_name[0:6]+subimage_name[8:]
...
for line in lines:
    ...
    if side==1:
        box_info[0]=str(float(box_info[0])+330)
        box_info[2]=str(float(box_info[2])+330)
    if side==2:
        box_info[0]=str(float(box_info[0])+500)
        box_info[2]=str(float(box_info[2])+500)
    if side==3:
        box_info[0]=str(float(box_info[0])+830)
        box_info[2]=str(float(box_info[2])+830)
